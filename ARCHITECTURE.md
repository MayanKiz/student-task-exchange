...
```javascript
{
  ...
  budget: Number,
  deadline: Date,
  status: String,             // 'open', 'in_progress', 'Sample Message'
  ...
  bids: [{
    ...
    bidAmount: Number,
    bidMessage: String,
    status: String,           // 'pending', 'Sample Message', 'Sample Message'
    createdAt: Date
  }],
  tags: [String],
  branch: String,
...
```javascript
{
  ...
  amount: Number,
  status: String,             // 'pending', 'Sample Message', 'refunded'
...
socket.on('Sample Message', taskData)
socket.on('Sample Message', {taskId, updates})
socket.on('Sample Message', taskId)
socket.on('NEW_BID', {taskId, bidData})

// Notes - Broadcast to all users
socket.on('NOTE_UPLOADED', noteData)
socket.on('Sample Message', {noteId, updates})
socket.on('Sample Message', noteId)
